...
def test_batch_concurrency(batch):
    """
    ...
    two_instance_sample = buffer.sample_batch()
...
def test_batch_shape(batch):
    ...
    assert np.shape(batch_sample.observations) == (config.buffer_config.batch_size, config.observation_dim)
    assert np.shape(batch_sample.actions) == (config.buffer_config.batch_size,)
    assert np.shape(batch_sample.rewards) == (config.buffer_config.batch_size,)
    assert np.shape(batch_sample.next_observations) == (config.buffer_config.batch_size, config.observation_dim)
    assert np.shape(batch_sample.dones) == (config.buffer_config.batch_size, )
