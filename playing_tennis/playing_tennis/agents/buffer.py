...
class ReplayBuffer:
    ...
    def __init__(self, config: BufferConfig):
        ...
        self.actions = np.zeros([config.action_dim, config.buffer_size], dtype=np.float32)
        self.rewards = np.zeros(config.buffer_size, dtype=np.float32)
        self.dones = np.zeros(config.buffer_size, dtype=bool)
    ...
    def store(self, observation: np.ndarray, action: np.ndarray, reward: np.float,
        ...
        self.pointer = (self.pointer + 1) % self.config.buffer_size
        self.size = min(self.size + 1, self.config.buffer_size)
    ...
    def sample_batch(self, batch_size: int) -> Batch:
        ...
        return Batch(self.observations[:, idxs].T, self.actions[:, idxs].T, self.rewards[idxs],
